...
**GET** `/v1/settlements/ondemand/:id`
...
```bash: CLI
razorpay settlements instant-fetch setlod_ABC123
```
...
* code: 400
* description: The settlement id was not found in the merchant's records. Distinct from `The id provided does not exist` — this fires when the id is well-formed but no matching settlement exists.
* solution: Pass a valid Instant Settlement id (prefixed `setlod_`) returned from a previous Create Instant Settlement call, or use the fetch-all endpoint to list available ids.
