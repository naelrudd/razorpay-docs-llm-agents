...
`customer_id`
: `string` The unique identifier of the customer the QR Code is linked with. Know more about the [Customers API](https://razorpay.com/docs/build/llm-docs/api/customers.md).
...
`close_reason`
: `string` Reason the QR code was closed. One of `on_demand` (closed via API), `paid` (paid amount reached), or `expired` (closed automatically after the close-by time).

`closed_at`
: `integer` Timestamp, in Unix format, when the QR code was closed. For example, `1778024203`.

`tax_invoice`
: `array` Tax invoice details for GST-compliant QR codes. An array containing GST identification and supply information.
