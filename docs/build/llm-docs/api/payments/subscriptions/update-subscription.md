...
Parameters to update can't be empty.
* code: 400
* description: The PATCH request body is empty. At least one updatable field must be provided.
...
The remaining count must be at least 1.
* code: 400
* description: `remaining_count` was passed as `0` or a negative value.
* solution: Pass `remaining_count` as a positive integer.

\{any extra field\} is/are not required and should not be sent.
* code: 400
* description: The request body contains fields that are not updatable on a Subscription.
...
* code: 400
* description: The Subscription was authenticated via Emandate. Emandate subscriptions cannot be updated through this API.
...
* code: 400
* description: A concurrent update, cancel or pause/resume operation is already running for the same Subscription. Razorpay holds a short-lived lock against the subscription id to prevent state-corruption.
...
Subscriptions with active offers can only be downgraded at the end of the billing cycle.
* code: 400
* description: A `plan_id` change has been requested that downgrades the Subscription amount, but the Subscription has an active offer attached. Downgrades on offer-linked subscriptions are only allowed at the end of the current billing cycle.
