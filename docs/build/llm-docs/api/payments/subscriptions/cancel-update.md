...
* code: 400
* description: There is no scheduled update on the subscription to cancel. This endpoint is only meaningful after a `PATCH /v1/subscriptions/:id` with a `schedule_change_at` value.
* solution: Schedule an update first via the Update Subscription endpoint, then call this endpoint to cancel it before it takes effect.

The id provided is invalid or could not be found.
* code: 400
* description: The subscription id in the URL is malformed (for example a wrong-prefix id like `pay_...`) or does not exist.
* solution: Pass a valid subscription id (`sub_`).
