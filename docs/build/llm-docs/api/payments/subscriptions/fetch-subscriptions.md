...
: `string` Status of the Subscription. Possible values: 
  ...
  Know more about [Subscriptions States](https://razorpay.com/docs/build/llm-docs/payments/subscriptions/states.md).  
...
`end_at`
: `integer` Timestamp, in Unix format, when the subscription ended.

`created_at`
: `integer` Timestamp, in Unix format, when the subscription was created. For example, `1778024127`.
...
* code: 400
* description: A query parameter that is not part of the fetch-subscriptions schema was passed.
