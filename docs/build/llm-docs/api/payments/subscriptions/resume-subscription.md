...
* code: 400
* description: Resume is only allowed for subscriptions in the `paused` state. A subscription in `created` (not yet authenticated by the customer) cannot be resumed.
* solution: Ensure the subscription is in the `paused` state before calling resume.

resume is not allowed, feature is not enabled.
* code: 400
* description: The resume-subscription feature has not been enabled on your account.
* solution: Contact [Razorpay support](https://razorpay.com/support/) to get the pause/resume feature enabled.

This subscription cannot be resumed at this time.
* code: 400
* description: The subscription is in a state from which it cannot be resumed (for example, the customer's recurring token has expired or the underlying mandate is no longer valid).
* solution: Fetch the subscription to check its current state and the status of its recurring token. If the token has expired, the customer needs to re-authenticate the subscription.

The ID provided is invalid or could not be found.
* code: 400
* description: The `subscription_id` passed in the URL is well-formed but does not exist or does not belong to the requesting merchant.
