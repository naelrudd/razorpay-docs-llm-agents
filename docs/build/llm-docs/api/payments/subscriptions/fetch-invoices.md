...
**GET** `/v1/invoices?subscription_id=:sub_id`
...
* code: 400
* description: The `subscription_id` query parameter is malformed (shorter than the expected `sub_` format).
* solution: Pass `subscription_id` as a well-formed subscription id (`sub_`).
