...
* solution: Ensure that the [Subscriptions feature](https://razorpay.com/docs/build/llm-docs/payments/subscriptions.md#flash-checkout) is enabled both in the test and live modes before creating a Subscription.
...
* code: 400
* description: The `plan_id` was not included in the request body.
* solution: Pass `plan_id` (the id of an active plan in the same account).

The plan id must be 19 characters.
* code: 400
* description: The value passed for `plan_id` is not 19 characters long (the expected `plan_` format).
* solution: Pass `plan_id` in the form `plan_`.

The plan id must be at least 14 characters.
* code: 400
* description: The value passed for `plan_id` is shorter than the minimum-id payload length. Returned for short or malformed plan ids.
* solution: Pass a well-formed `plan_id`.

The total count field is required when end at is not present.
* code: 400
* description: Neither `total_count` nor `end_at` was included in the request body. At least one is required to bound the subscription.
* solution: Pass either `total_count` (number of billing cycles) or `end_at` (UNIX timestamp).

The total count must be at least 1.
* code: 400
* description: `total_count` was passed as `0` or a negative value.
* solution: Pass `total_count` as a positive integer (1 or higher).

The total count must be an integer.
* code: 400
* description: A non-integer value was passed for `total_count`.
* solution: Pass `total_count` as an integer.

The quantity must be at least 1.
* code: 400
* description: `quantity` was passed as `0` or a negative value.
* solution: Pass `quantity` as a positive integer (1 or higher).

start_at cannot be lesser than the current time.
* code: 400
* description: `start_at` is set to a past UNIX timestamp.
* solution: Pass `start_at` as a UNIX timestamp in the future.

The customer notify field must be true or false.
* code: 400
* description: A non-boolean value (for example a string like `"maybe"`) was passed for `customer_notify`.
...
* code: 400
* description: The request body contains fields that are not part of the Subscriptions API schema.
...
* code: 400
* description: Both `end_at` and `total_count` were sent together. The subscription end can be defined either by an explicit end date or by a count of cycles, not both.
