...
* code: 400
* description: The subscription has not yet been authenticated by the customer. Pause is only allowed for subscriptions in the `active` state.
* solution: Wait until the customer authenticates the subscription (state transitions to `active`) before attempting to pause it.

pause is not allowed, feature is not enabled.
* code: 400
* description: The pause-subscription feature has not been enabled on your account.
