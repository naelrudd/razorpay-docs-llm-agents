...
`checkout`
: `object` Checkout configuration applied to this order, when the order was created with checkout-time options (theme, prefill, partial-payment settings).

`transfers`
: `object` Collection of transfers associated with this order when Route is enabled. Contains entity type, count, and items array.

`receipt`
: `string` Receipt number that corresponds to this order, set by the merchant. Up to 40 characters. For example, `audit_1778024126`.

`amount_paid`
: `integer` The amount paid for this order, in the smallest currency unit (paise for INR). For example, `0`.

`amount_due`
: `integer` The amount due for this order, in the smallest currency unit (paise for INR). For example, `50000`.

`attempts`
: `integer` Number of payment attempts made for this order. For example, `0`.
