...
customer, line_items, sms_notify, email_notify, draft, date, currency is/are not required and should not be sent
* code: 400
* description: The mentioned parameters are not required for updating an invoice. The API echoes the rejected field name in the response.
...
* code: 400
* description: The invoice already has one or more `line_items`. Razorpay derives the invoice amount from the line items, so a top-level `amount` cannot be updated independently.
