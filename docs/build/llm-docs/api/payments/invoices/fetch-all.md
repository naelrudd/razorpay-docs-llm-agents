...
`subscription_id`
: `string` Unique identifier of the subscription associated with this invoice.
...
`idempotency_key`
: `string` A unique key used to ensure idempotency of the invoice creation request, preventing duplicate invoices.
...
* code: 400
* description: The `customer_id` query parameter is malformed (for example a string that doesn't match the `cust_` pattern, or a wrong-prefix id like `pay_...`). Note: a well-formed but non-existent `customer_id` returns an empty collection (HTTP 200, `count: 0`), not an error.
* solution: Pass `customer_id` in the form `cust_`.

The payment id must be at least 14 characters.
* code: 400
* description: The `payment_id` query parameter is shorter than the expected 14-character payload (or otherwise malformed). A well-formed but non-existent `payment_id` returns an empty collection instead of an error.
* solution: Pass `payment_id` in the form `pay_`.

Not a valid type.
* code: 400
* description: The value passed for the `type` query parameter is not one of the supported invoice types. The API echoes the rejected value, for example `Not a valid type: invoiceee`.
