...
You can use the [Orders](https://razorpay.com/docs/build/llm-docs/payments/orders.md) API to create a unique Razorpay `order_id` that is associated with the authorisation transaction. The following endpoint creates an order.
...
: Details related to the authorisation transaction such as max amount and bank account information. Pass a value in the `first_payment_amount` parameter if you want to auto-charge the customer the first payment immediately after authorization.
 ...
 `auth_type` _mandatory_
  ...
  : `integer` Use to set the maximum amount per debit request. Know about [maximum and default values](https://razorpay.com/docs/build/llm-docs/payments/recurring-payments/paper-nach/faqs.md#3-is-there-a-limit-on-the-debit).
...
You can register a customer's mandate **and** charge them the first recurring payment as part of the same transaction. Refer to the [Paper NACH section under Registration and Charge First Payment Together](https://razorpay.com/docs/build/llm-docs/api/payments/recurring-payments/paper-nach/auto-debit.md#1-create-an-authorization-transaction) for more information.
...
: Details of the authorisation payment.
  ...
  `max_amount` _optional_
  : `integer` Use to set the maximum amount, in paise, per debit request. Know about [maximum and default values](https://razorpay.com/docs/build/llm-docs/payments/recurring-payments/paper-nach/faqs.md).
...
Know more about [Tokens](https://razorpay.com/docs/build/llm-docs/payments/recurring-payments/paper-nach/integrate.md#fetch-nach-mandate-registration-details).
