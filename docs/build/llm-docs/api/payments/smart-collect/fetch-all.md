...
: `json object` Configuration of desired receivers for the Customer Identifier.
  ...
  `notes`
  : `json object` Any custom notes you might want to add to the virtual bank account can be entered here. Know more about [notes](https://razorpay.com/docs/build/llm-docs/api/understand.md#notes). This parameter appears in the response only when `bank_account` is passed as the receiver `type`.
