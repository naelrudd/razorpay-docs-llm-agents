...
: `object` The device details.
  ...
  `content`
  : `string` SMS content to be used for device binding of this token as returned by the [Get Verification Token API](https://razorpay.com/docs/build/llm-docs/api/payments/tpap-pro/customer-onboarding/verification-token.md).

`device` _mandatory_
: `object` The device information. This should be the same information sent when you [get the verification token api](https://razorpay.com/docs/build/llm-docs/api/payments/tpap-pro/customer-onboarding/verification-token.md).
...
`status`
: `string` Device binding status. Know more about the [device binding status](https://razorpay.com/docs/build/llm-docs/payments/tpap-pro/integration-guide.md#device-binding-status).
