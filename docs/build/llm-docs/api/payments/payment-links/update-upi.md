...
: `boolean` Indicates whether customers can make [partial payments](https://razorpay.com/docs/build/llm-docs/payments/payment-links/partial-payments.md) using the Payment Link. Partial payments are not allowed with UPI Payment Links. Possible values:
...
* code: 400
* description: Another update or cancel operation is currently in progress for the same Payment Link. Razorpay holds a short-lived lock against the link id to prevent concurrent state changes.
...
* code: 400
* description: The request body contains a field that is not editable while the Payment Link is in the current status. The error message echoes the actual status. For example, `extra fields sent in state partially_paid`. The set of editable fields varies based on the link's status.
