...
razorpay payment-links create \
  ...
  --currency INR \
  --accept-partial \
  --first-min-partial-amount 500 \
  --label-min-amount "Minimum Due" \
  --label-partial-amount "Pay Partial" \
  --label-partial-description "Enter a partial amount" \
  --label-full-amount "Pay Full Amount"
```
