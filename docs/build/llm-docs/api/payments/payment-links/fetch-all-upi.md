...
`payment_links`
: `array` Array of UPI payment link objects matching the query parameters.
...
* code: 400
* description: The `reference_id` query parameter exceeds the 40-character limit. The error message references `receipt` because that is the internal field name `reference_id` maps to.
* solution: Pass `reference_id` as a string of 40 characters or fewer.
