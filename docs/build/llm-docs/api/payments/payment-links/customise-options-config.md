...
- These parameters must be passed along with the `options` and `checkout` parameters mentioned in the Request Parameters section below, along with the [Create Payment Link API](https://razorpay.com/docs/build/llm-docs/api/payments/payment-links.md#create-payment-link) parameters.
