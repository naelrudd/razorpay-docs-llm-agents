...
razorpay payment-links create \
  --amount 1000 \
  --currency INR \
  --hosted-label-receipt "Invoice No" \
  --hosted-label-description "Order Info" \
  --hosted-label-amount-payable "Total Due" \
  --hosted-label-amount-paid "Amount Received" \
  --hosted-label-expire-by "Valid Until" \
  --hosted-label-amount-due "Balance Due"
```
...
: `array` Parent parameter under which the `hosted_page` and `label` child parameters must be passed.
    ...
    `hosted_page` _mandatory_
    : `array` The parameter for the Payment Details section. `label` is its child parameter.

        `label`
        : `array` Modifies the labels of fields in the Payment Details section.
