...
- Verify the `razorpay_signature` parameter to validate that it is authentic and sent from Razorpay servers. Know more about how to [Verify Signature](https://razorpay.com/docs/build/llm-docs/api/payments/payment-links.md#verify-signature).
...
* solution: Ensure that you use [Live API keys](https://razorpay.com/docs/build/llm-docs/api/authentication.md#live-mode-api-keys) while passing UPI links.
...
* code: 400
* description: `accept_partial: true` (or any `first_min_partial_amount`) was passed alongside `upi_link: true`. Partial payments are not supported for UPI links.
...
* code: 400
* description: A decimal value was passed for `amount`. Amounts must be integers in currency subunits.
...
* code: 400
* description: A UPI Payment Link was requested but UPI is not enabled as a payment method on the merchant account.
