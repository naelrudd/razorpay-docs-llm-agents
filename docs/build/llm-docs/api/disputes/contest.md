...
- Attaching an evidence document using [Documents API](https://razorpay.com/docs/build/llm-docs/api/documents.md) with having a purpose set to `dispute_evidence`.
...
List shipping_proof = new ArrayList<>();
...
JSONObject disputeRequest = new JSONObject();
List billing_proof = new ArrayList<>();
