...
Ensure you [allowlist IPs](https://razorpay.com/docs/build/llm-docs/x/dashboard/allowlist-ip.md) and pass the [idempotency key](https://razorpay.com/docs/build/llm-docs/api/x/payout-idempotency/make-request.md) to make a successful payout.

 Payout to Cards is only available for PCI DSS compliant merchants. To enable the feature, [raise a request](https://razorpay.com/docs/build/llm-docs/x/support.md) on the RazorpayX Dashboard. Refer to the [payouts to cards](https://razorpay.com/docs/build/llm-docs/api/x/payouts-cards.md) documentation. 
...
- Contact
  ...
  - Use the [Update Contact API](https://razorpay.com/docs/build/llm-docs/api/x/contacts/update.md) if you want to make changes to an existing contact.
...
To understand the status of the payouts, refer to [Payout Status Details](https://razorpay.com/docs/build/llm-docs/errors/x/payout-status-details.md).
...
: `object` Contact and fund account details to which the payout was made.
  ...
  : `string` The type of fund account being created. It can be a `bank_account`, `vpa`, `card`.
    ...
    : `object` Details of the credit card that is being used to create the fund account.
        ...
        `issuer`
        : `string` The name of bank that issued the card. For example, `HDFC`. Refer to the [Supported Banks and Payout Modes section](https://razorpay.com/docs/build/llm-docs/api/x/payouts-cards.md#supported-banks-and-payout-modes) section for more details.
...
: `string` The payout status. Possible payout states:
  - `queued`
  - `pending` (if you have [Approval Workflow](https://razorpay.com/docs/build/llm-docs/x/manage-teams/approval-workflow.md) enabled)
  - `rejected` (if you have [Approval Workflow](https://razorpay.com/docs/build/llm-docs/x/manage-teams/approval-workflow.md) enabled)
  - `processing`
  - `processed`
  - `cancelled`
  - `reversed`
...
`mode`
: `string` The mode used to make the payout. Refer to the [Payouts section](https://razorpay.com/docs/build/llm-docs/x/payouts.md#payout-modes) for more details. Available modes:
    - `NEFT`
    - `RTGS`
    - `IMPS`
...
: `object` This parameter returns the current status of the payout. For example, `IMPS is not enabled on beneficiary account, Retry with different mode.`
    ...
    `reason`
    : `string` The error reason. For example, `imps_not_allowed`. [Payout Status Details and Next Steps](https://razorpay.com/docs/build/llm-docs/errors/x/payout-status-details.md).
