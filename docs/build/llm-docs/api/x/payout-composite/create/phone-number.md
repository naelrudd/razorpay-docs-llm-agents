...
: `object` This parameter returns the current status of the payout. For example, `IMPS is not enabled on beneficiary account, Retry with different mode.`
  ...
  `reason`
  : `string` Reason for the current status. For example, `imps_not_allowed`. [Payout Status Details and Next Steps](https://razorpay.com/docs/build/llm-docs/errors/x/payout-status-details.md).
