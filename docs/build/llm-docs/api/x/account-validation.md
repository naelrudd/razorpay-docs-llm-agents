...
[Account Validation](https://razorpay.com/docs/build/llm-docs/x/fund-account-validation.md) validates your Contact's Fund Account details (such as bank account or virtual payment address (VPA) details) to verify the account number where the user wants to receive the amount. Account Validation is not available in test mode and is possible only for RazorpayX Lite.
...
Fork the Razorpay Postman Public Workspace and try the Account Validation APIs using your [Test API Keys](https://razorpay.com/docs/build/llm-docs/x/dashboard/api-keys.md).
