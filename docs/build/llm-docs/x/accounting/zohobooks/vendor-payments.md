...
Zoho Books may require more information than originally provided while making a vendor payment. With this integration and the initial configuration, the process of filling up the required details is automated. You can make configurations with [**Rules & Configurations**](https://razorpay.com/docs/build/llm-docs/x/accounting/zohobooks.md#rules-configuration) under General, Account Mapping and Contact Mapping to start. [Vendor Payments](https://razorpay.com/docs/build/llm-docs/x/vendor-payments.md) are recorded as **Bills** under Accounting. They are recorded here as soon as the invoice is created. The invoice need not be in `paid` state for the sync to occur.
...
You can automate this process to an extent by setting [**Contact Rules**](https://razorpay.com/docs/build/llm-docs/x/accounting/zohobooks.md#rules). This means that you can select a corresponding **Zoho Books Expense Account** to which you want the vendor payments made to that particular vendor, to sync. 
