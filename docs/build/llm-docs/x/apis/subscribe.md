...
Webhooks (Web Callback, HTTP Push API or Reverse API) are one of the ways in which one web application can send information to another application in real-time when a specific event happens. Know more about [Webhooks](https://razorpay.com/docs/build/llm-docs/webhooks.md).

## Set Up and Edit Payout Webhooks
You must first [set up and edit Payout Webhooks](https://razorpay.com/docs/build/llm-docs/webhooks/setup-edit-payouts.md) as per your requirement.

## Validate and Test Webhooks
Then you must [validate and test webhooks](https://razorpay.com/docs/build/llm-docs/webhooks/validate-test.md) as a best practice. 
...
When your webhook `secret` is set, Razorpay uses it to create a hash signature with each payload. This hash signature is passed with each request under the `X-Razorpay-Signature` header that you need to validate at your end. Support for validating the signature is provided in all our  [language SDKs](https://razorpay.com/docs/build/llm-docs/payments/server-integration.md).
...
See [Payout Sample Payloads](https://razorpay.com/docs/build/llm-docs/webhooks.md#payout-payloads).

For transaction webhook events, check the [Transaction Sample Payloads](https://razorpay.com/docs/build/llm-docs/webhooks.md#transaction-payloads).

### Related Information

- [Set Up Webhooks for Payouts](https://razorpay.com/docs/build/llm-docs/webhooks/setup-edit-payouts.md)
- [Payout APIs](https://razorpay.com/docs/build/llm-docs/api/x/payouts.md)
