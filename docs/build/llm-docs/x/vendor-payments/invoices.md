...
On Vendor Payments, you can make payouts to vendors against the invoices you receive from them. You can receive and import the invoice through email, [Vendor Portal](https://razorpay.com/docs/build/llm-docs/x/vendor-payments/portal/business.md), upload the PDF on RazorpayX Dashboard or enter the details manually.
...
4. Select an existing vendor or [create a new vendor](https://razorpay.com/docs/build/llm-docs/x/contacts.md#create-a-contact). The vendor is the person or entity to which you are making an payment to, basing an invoice received.
5. Select a Fund account for the vendor or [create a new Fund account](https://razorpay.com/docs/build/llm-docs/x/fund-accounts.md#create-a-contact-with-fund-account). These are the account details (bank account or UPI ID) to which the amount is transferred.
...
Once you make the payment, it follows the [Approval Workflow](https://razorpay.com/docs/build/llm-docs/x/manage-teams/approval-workflow.md) set on your RazorpayX account.
...
- Invoices marked as `paid` are considered for TDS payments. You can calculate and pay the TDS using [Tax Payments](https://razorpay.com/docs/build/llm-docs/x/tax-payments.md).
