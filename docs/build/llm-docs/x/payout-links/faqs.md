...
### 1. Is the Payout Link supported in test mode?
         ...
         No, currently it is not supported in [Test mode](https://razorpay.com/docs/build/llm-docs/x/dashboard/test-mode.md). You can only make requests or designate links in Live mode.
...
### 2. What are the supported modes for creating Payout Links?
         ...
         You can create Payout Links via:
            - [Payout Link APIs](https://razorpay.com/docs/build/llm-docs/api/x/payout-links.md).
            - [RazorpayX Dashboard](https://x.razorpay.com/).
            - [Bulk upload feature](https://razorpay.com/docs/build/llm-docs/x/payout-links/bulk.md).
...
### 4. Is the maker-checker applicable for Payout Links?
         ...
         Yes, if the [maker-checker](https://razorpay.com/docs/build/llm-docs/x/manage-teams/approval-workflow.md) is enabled on the account, then it will be applicable. 
...
### 7. If a new link is issued to an old customer, will the customer need to enter the fund account details again?
         ...
         You can manually [resend the Payout Link](https://razorpay.com/docs/build/llm-docs/x/payout-links/create.md#create-a-payout-link) from the Payout Link summary pop-up page. 
        

    
### 8. Can a Payout link be cancelled?

         If the Fund Account details are incorrect or the Payout Link is no longer relevant, you can cancel the Payout Link via the [Dashboard](https://razorpay.com/docs/build/llm-docs/x/payout-links/create.md#other-actions) or by using the [Cancel a Payout Link API](https://razorpay.com/docs/build/llm-docs/api/x/payout-links/cancel.md).

         Only Payout Links in the `issued` state can be cancelled. Know more about [Payout Link Statuses](https://razorpay.com/docs/build/llm-docs/x/payout-links/life-cycle.md). If [Approval Workflow](https://razorpay.com/docs/build/llm-docs/x/manage-teams/approval-workflow.md) is enabled, [cancel the Payout](https://razorpay.com/docs/build/llm-docs/x/payouts.md#cancel-payouts) first and then the Payout Link. 
...
### 11. Does the Payout Link support an expiry time?
         ...
         Yes, you can [set expiry](https://razorpay.com/docs/build/llm-docs/x/payout-links/set-expiry.md#set-expiry) date and time on the Payout Link.
        

    
### 12. What happens to Payout Links if funds in a debit account are less?
...
### 14. Is IP Allowlisting mandatory for creating Payout Links?
        ...
        [Allowlisting your IP](https://razorpay.com/docs/build/llm-docs/x/dashboard/allowlist-ip.md) addresses is mandatory for creating Payout Links through APIs. However, it is not mandatory for creating Payout Links through the Dashboard.
...
### 2. How can I set expiry for Bulk Payout Links?
         ...
         To set expiry for Bulk Payout Links, you must [enable expiry](https://razorpay.com/docs/build/llm-docs/x/payout-links/set-expiry.md#enable-expiry) on the RazorpayX Dashboard. You can then [set expiry](/x/payout-links/bulk#how-it-works) for bulk Payout Links in the sample template.
