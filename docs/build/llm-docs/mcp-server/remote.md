...
- **Authentication**: Generate [Razorpay API keys](https://razorpay.com/docs/build/llm-docs/payments/dashboard/account-settings/api-keys.md#generate-api-keys) from the Dashboard. Alternatively, you can use [OAuth](https://razorpay.com/docs/build/llm-docs/mcp-server/oauth.md).
...
Effective **August 13, 2025**, the endpoint `https://mcp.razorpay.com/sse` is deprecated and replaced by `https://mcp.razorpay.com/mcp`, which supports streamable HTTP responses for improved performance. Update your integrations immediately to avoid service disruptions. Refer to the [changelog](https://razorpay.com/docs/build/llm-docs/api/changelog.md).

  - **Claude**: Connect using the official Razorpay connector or Claude Code (CLI).

  - **ChatGPT**: Install the official Razorpay app and authorise it using OAuth.

  - **Cursor**: Add the server through the mcp-remote bridge in mcp.json.

  - **Visual Studio Code**: Configure agent mode with a secure merchant token prompt.

  - **Windsurf**: Add the server to the Cascade agent via mcp_config.json.

  - **Replit**: Add the server from Integrations settings and authorise via OAuth.

  - **Gemini CLI**: Add the server to the mcpServers block in settings.json.
...
1. **Explore Use Cases**: Review [practical examples](https://razorpay.com/docs/build/llm-docs/mcp-server/use-cases.md) of AI-powered payment operations.
2. **Learn Available Tools**: Check the [Tools Reference](https://razorpay.com/docs/build/llm-docs/mcp-server/tools-reference.md) for complete capabilities.
3. **Advanced Configuration**: See [Configuration Guide](https://razorpay.com/docs/build/llm-docs/mcp-server/configuration.md) for additional options.
