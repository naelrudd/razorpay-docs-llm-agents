...
Integrate with [Orders API](https://razorpay.com/docs/build/llm-docs/api/orders.md) to create an order. Given below is the sample code for Create an Order API. Pass the `account_id` of the sub-merchant using `X-Razorpay-Account` in the header.
...
Check [our SDKs](https://razorpay.com/docs/build/llm-docs/payments/server-integration.md) for the supported platforms.
...
You can view the payments received from customers on the sub-merchant's  Dashboard. Also, you can retrieve the status of the payments by polling our [APIs](https://razorpay.com/docs/build/llm-docs/api.md).
...
- [Razorpay Payment Gateway: Web Standard Integration Guide](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/web-integration/standard.md).
