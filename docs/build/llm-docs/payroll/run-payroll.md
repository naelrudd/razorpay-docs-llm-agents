...
After you have finalised the salary components, you can finally execute your payroll. Ensure you refer to the [payroll checklist](https://razorpay.com/docs/build/llm-docs/payroll/execute-payroll.md) before doing so. 
...
1. Check the following: 
    ...
    - Fulfil other components from the [execute payroll checklist](https://razorpay.com/docs/build/llm-docs/payroll/execute-payroll.md).
...
You have successfully executed payroll. You cannot make [payroll modifications](https://razorpay.com/docs/build/llm-docs/payroll/exceptional-cases.md#modify-salary-after-payroll-is-executed) after you execute payroll. 
...
1. On the **Revise compensation** page, modify the following as applicable: 
    1. CTC in case of default salary structure adopted by Payroll. Or you can select [custom salary structure](https://razorpay.com/docs/build/llm-docs/payroll/salary.md#setup-salary-structure) as applicable to your organisation. 
...
1. Enter the revised salary details against the relevant employee's names. 
    ...
    - In the Custom Structure sheet, enter the revised CTC breakdown for each employee under [Custom Salary Structure](https://razorpay.com/docs/build/llm-docs/payroll/salary.md#setup-salary-structure). 
...
1. Select the **Last working date** (LWD) from the drop-down calendar and provide a reason for dismissal. If you have enabled the [resignation module](https://razorpay.com/docs/build/llm-docs/payroll/employees/resignation.md), we automatically add the LWD once the employee submits their resignation.

This initiates the full and final settlement process for the employee. Know how to [process employee's FNF](https://razorpay.com/docs/build/llm-docs/payroll/resignation.md). 
...
You have three ways with which you can calculate [loss of pay](https://razorpay.com/docs/build/llm-docs/payroll/leaves.md#loss-of-pay-for-unpaid-excessive-leave-work) for Unpaid/Excessive Leave Work. Know how you can make salary deductions for the same below. 
...
If the edit icon looks uneditable and says **Final**, click **Make Changes** on the right menu to make the column editable again. Know more about [making changes to payroll](https://razorpay.com/docs/build/llm-docs/payroll/exceptional-cases.md#modify-salary-after-payroll-is-executed).

### Loss of Pay
Payroll does not automatically deduct the loss of pay based on attendance. By default, attendance is considered `present` for all calendar days of the month in Payroll. Know more about [attendance](https://razorpay.com/docs/build/llm-docs/payroll/attendance.md). 
...
- **Jibble Integration**: Sync the loss of pay data from the Jibble Dashboard to automatically calculate and deduct loss of pay amount. Know more about the [Jibble attendance integration](https://razorpay.com/docs/build/llm-docs/payroll/integrations/jibble.md). 
...
- [One-time Payments](https://razorpay.com/docs/build/llm-docs/payroll/one-time-payments.md) 
