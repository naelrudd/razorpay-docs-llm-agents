...
- API access is required for UbiAttendance integration. [Contact Support](https://razorpay.com/docs/build/llm-docs/payroll/plans.md#contact-support) for API aceess.
...
1. Request access to RazorpayX Payroll APIs. 
    [Contact Support](https://razorpay.com/docs/build/llm-docs/payroll/plans.md#contact-support) to request access to Payroll APIs. Integration with UbiAttendance requires Payroll's API ID and Key.
...
- Additions to payroll arising due to overtime payments to employees must be manually added as [additions](https://razorpay.com/docs/build/llm-docs/payroll/run-payroll.md#additions) on the Payroll Dashboard. UbiAttendance does not automatically transfer the additions data to Payroll.
