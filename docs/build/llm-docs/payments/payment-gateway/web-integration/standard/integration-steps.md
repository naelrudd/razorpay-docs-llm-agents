...
The callback_url Checkout option is only for WebView and redirect flows. For standard web integrations, use the handler function to handle payment completion on the client side, and webhooks for server-side confirmation. See [Webhooks](https://razorpay.com/docs/build/llm-docs/webhooks.md) for details.
