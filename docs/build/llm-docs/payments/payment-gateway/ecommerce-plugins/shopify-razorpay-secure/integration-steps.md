...
Follow the steps given below to integrate All-in-one Razorpay Payment Gateway on your Shopify store.
...
- Create a [Razorpay account](https://razorpay.com/docs/build/llm-docs/payments/set-up.md).
- Complete KYC. Refer to the list of [documents](https://razorpay.com/docs/build/llm-docs/payments/business-types-kyc-documents.md) required.
- You will receive a confirmation email and WhatsApp message when your account is activated. You can use test mode to test the integration till your KYC is completed.
...
Make sure you log in with **owner** credentials to connect Razorpay with Shopify successfully.
         ...
         Razorpay Secure now appears as a Payment Gateway on your Shopify Store checkout. This completes your integration. For more information, see [Shopify FAQs](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/ecommerce-plugins/shopify-razorpay-secure/faqs.md).
...
Test mode in Shopify is a separate toggle from your Razorpay Dashboard. Enabling test mode on the Razorpay Dashboard alone is not enough. You must enable it on Shopify as well (Settings → Payments → Manage → Enable test mode).
...
### 3.1 Switch from Test mode to Live mode
         ...
         3. On the **Supported payment methods** section, click **Manage** on the **All-in-one Razorpay Payment Gateway** app.
...
Before you go live:
- Ensure you have turned off the **Enable test mode** option on Shopify (Settings → Payments → Manage)
- Ensure your Razorpay account is in Live Mode with Live API keys configured.

Missing either step will result in payment failures in real-time.
