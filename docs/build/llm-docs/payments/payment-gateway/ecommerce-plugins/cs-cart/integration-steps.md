...
Webhook is auto-configured when you enter and save the API key ID and secret on the plugin settings page. You need to verify that webhooks are enabled on your Razorpay [Dashboard](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/ecommerce-plugins/cs-cart/troubleshooting-faqs.md#2-how-can-i-verify-if-webhooks-are). However, for versions lower than 1.4.0, you need to [manually configure webhooks](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/ecommerce-plugins/cs-cart/troubleshooting-faqs.md#1-my-webhooks-are-not-auto-configured-since-i).
