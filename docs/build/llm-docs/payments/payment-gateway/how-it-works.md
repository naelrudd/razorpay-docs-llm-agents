...
### 2. Create Order from Server
         ...
         For each order placed by your customer, use the [Razorpay Orders API](https://razorpay.com/docs/build/llm-docs/api/orders.md) to create an order from your server.
        

    
### 3. Order ID Returned

         Razorpay processes the details sent and returns an `order_id` to you, for example, `order_EKwxwAgItmmXdp`. Map this `order_id` to the transaction_id `#trn-345` you created in the first step. Know more about [order states](https://razorpay.com/docs/build/llm-docs/payments/orders.md#order-states).
...
### 6. Bank Authenticates the Payment
         ...
         Know more about [payment states](https://razorpay.com/docs/build/llm-docs/payments/payments.md#payment-life-cycle).
...
After the payment is successfully captured, the amount is settled in your account according to your settlement cycle. Know more about [settlements](https://razorpay.com/docs/build/llm-docs/payments/settlements.md).

### Refunds

Customers may claim [refunds](https://razorpay.com/docs/build/llm-docs/payments/refunds.md) after the transaction. In such cases, you can initiate a refund, post which the funds are sent back to the customer's account.

## Test Cards

Use the [test cards](https://razorpay.com/docs/build/llm-docs/payments/payments/test-card-details.md) to test domestic payments, international payments and subscriptions.
...
[Get started with Razorpay Payment Gateway integration.](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway.md) 
