...
Third-party validation (TPV) of bank accounts is mandatory for businesses in the BFSI (Banking, Financial Services and Insurance) sector that deal with Securities, Brokerage and Mutual Funds. You can accept customer payments using the Turbo UPI UI with TPV SDK. Know more about [how TPV works](https://razorpay.com/docs/build/llm-docs/payments/third-party-validation.md#how-it-works).

**WARN**

**Watch Out!**

To whitelist the customer account, use our S2S [Customer Add Bank Account API](https://razorpay.com/docs/build/llm-docs/api/customers/bank-accounts.md#1-add-bank-account-of-customer). This optional functionality ensures that only pre-approved accounts are displayed to users during the onboarding process, streamlining the experience and enhancing security.
...
### Prerequisites
         ...
         3. Integrate with the [Razorpay iOS Custom SDK](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/ios-integration/custom/build-integration.md).
...
- Use the `rzp_test_8UzRYt0d70Ntgz` API key id for testing on the UAT environment and the [Razorpay live keys](https://razorpay.com/docs/build/llm-docs/api/authentication.md#live-mode-api-keys) for prod testing.
