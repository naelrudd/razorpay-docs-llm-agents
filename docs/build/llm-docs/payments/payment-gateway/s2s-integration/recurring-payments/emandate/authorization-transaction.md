...
You can register a customer's mandate **and** charge them the first recurring payment as part of the same transaction. Know more about [charge first payment automatically after Emandate registration](https://razorpay.com/docs/build/llm-docs/api/payments/recurring-payments/emandate/auto-debit.md).
...
`bank` _mandatory_
: `string` The customer's bank name. The bank code used here should match the bank details used in [Step 1.1.2](#112-create-an-order). Use the [Method API](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/s2s-integration/payment-methods/methods-api.md) to check the bank code.
...
If the payment request is valid, the response contains the following fields. Refer to the [S2S Json V2 integration document](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/s2s-integration/json/v2.md#step-2-create-a-payment_) for more details.
