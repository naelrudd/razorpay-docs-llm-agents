...
-d '{
  "amount": 100,
  "currency": "INR",
  "customer_id": "cust_SILnnXBQupdNrV",
  "method": "wallet",
  "token": {
    "max_amount": 1000000,
    "expire_at": 1798636800,
...
```json: Response
{
  ...
  "amount_paid": 0,
  "attempts": 0,
  "created_at": 1775121749,
  "currency": "MYR",
  "entity": "order",
  "id": "order_SYZb4bg9McB3iH",
  "method": null,
  "notes": {
    "notes_key_1": "Tea, Earl Grey, Hot",
    "notes_key_2": "Tea, Earl Grey… decaf."
...
`method` _optional_
: `string` Payment method used to make the authorisation transaction. Here, it is `wallet`.
...
Once an order is created, your next step is to create a payment. Use the below endpoint to create a payment with payment method as `wallet`.
...
`method` _mandatory_
: `string` The payment method selected by the customer. Here, the value must be `wallet`.
...
### Response Parameters
     ...
     If the payment request is valid, the response contains the following fields. Refer to the [S2S Json V2 integration document](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/s2s-integration/json/v2.md#step-2-create-a-payment) for more details.
...
After this step, you can proceed to integrate with the [Fetch Token API](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/s2s-integration/recurring-payments/wallets/tokens.md).
...
When you create a registration link, an [invoice](https://razorpay.com/docs/build/llm-docs/payments/invoices.md) is automatically issued to the customer. The customer can use the invoice to make the Authorisation Payment.
...
A registration link must always have the amount (in currency subunits) that the customer will be charged when making the authorisation payment. For wallets, the amount must be a minimum of 1.
...
subscriptionRegistration.put("method","wallet");
...
$api->subscription->createSubscriptionRegistration(array('customer'=>array('name'=>'Gaurav Kumar','email'=>'gaurav.kumar@example.com','contact'=>'+919876543210'),'type'=>'link','amount'=>100,'currency'=>'INR','description'=>'Registration Link for Gaurav Kumar','subscription_registration'=>array('method'=>'wallet','max_amount'=>'1000000','expire_at'=>'1634215992','frequency'=>'monthly'),'receipt'=>'Receipt No. 5','email_notify'=> true,'sms_notify'=>true,'expire_by'=>1634215992, 'notes'=> array('note_key 1'=> 'Beam me up Scotty','note_key 2'=> 'Tea. Earl Gray. Hot.')));
```javascript: Node.js
var instance = new Razorpay({ key_id: 'YOUR_KEY_ID', key_secret: 'YOUR_SECRET' })
...
client.registration_link.create({
    ...
    'subscription_registration': {'method': 'wallet', 'max_amount': '1000000'
...
subscriptionRegistration.Add("method", "wallet");
...
: `object` Details of the authorisation transaction.
  ...
  `method` _mandatory_
  : `string` The authorisation method. Here it is `wallet`.
...
After this step, you can proceed to integrate with the [Fetch Token API](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/s2s-integration/recurring-payments/cards/tokens.md).
