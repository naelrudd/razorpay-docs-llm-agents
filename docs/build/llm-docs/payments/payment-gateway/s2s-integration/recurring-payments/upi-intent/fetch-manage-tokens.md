...
- Tokens have a lifecycle with defined states. Always check the token state before attempting a subsequent debit. Know more in the [Token States](#32-token-states) section below.
...
## 3.4. Fetch Tokens by Customer ID
...
Deleting a token removes it from Razorpay's database. The deleted token will not appear on the Dashboard or when all tokens are fetched. However, it does not cancel the mandate. If you wish to delete the mandate with Razorpay, you must first cancel it using the [Cancel Token API](#35-cancel-token).
