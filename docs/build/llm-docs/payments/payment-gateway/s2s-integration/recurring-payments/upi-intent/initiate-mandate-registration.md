...
Once an order is created, your next step is to create a payment to initiate the mandate registration on the customer's UPI app. Use the below endpoint to create a payment with payment method `upi`.
...
`upi`
: Details of the UPI payment flow.
...
### Response Parameters
       ...
       If the payment request is valid, the response contains the following fields. Refer to the [UPI Intent Flow document](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/s2s-integration/payment-methods/upi/intent.md) for more details.
       
`razorpay_payment_id`
: `string` Unique reference for the payment created. For example, `pay_EAm09NKReXi2e0`.

`link`
: `string` The UPI intent link to redirect the customer to their UPI app for mandate approval. Use this as a deeplink on mobile or convert it to a QR code for desktop.
