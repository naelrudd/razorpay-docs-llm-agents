...
1. [Create a Customer](#111-create-a-customer) - This allows Razorpay to uniquely identify the customer and store UPI Autopay tokens against the customer ID.
1. [Create an Order](#112-create-an-order) - Send the token details required to set up UPI Autopay on the customer ID.
1. [Create an Authorisation Payment](#113-create-an-authorisation-payment) - Initialise the authorisation journey where the actual mandate registration happens.

### 1.1.1 Create a Customer

Razorpay links recurring tokens to customers using a unique identifier generated through the Customer API. You can create customers with basic details such as `email` and `contact` and use them for various Razorpay offerings. Know more about [Customers](https://razorpay.com/docs/build/llm-docs/api/customers.md).
...
Use the [Orders API](https://razorpay.com/docs/build/llm-docs/api/orders.md) to create a unique Razorpay `order_id` that is associated with the mandate registration. The following endpoint creates an order.
...
The `token.max_amount` value must respect the limits defined by NPCI for your merchant category. For regular industries, the maximum is ₹99,999. For lending and investment categories (MCCs 6211, 6300, 7322, 6529, 5960), the maximum is ₹2,00,000. Refer to the [UPI Autopay overview](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/s2s-integration/recurring-payments/upi.md) for the complete limits table.
...
Set the `max_amount` as close to the actual debit amount as possible. For example, if the expected debit amount is ₹99-₹299, setting the `max_amount` to ₹5,000 may cause the customer to drop off due to trust issues. A realistic `max_amount` builds customer trust while allowing flexibility for variable debits.
...
### Response Parameters
       ...
       If the payment request is valid, the response contains the following fields. Refer to the [UPI Intent Flow document](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/s2s-integration/payment-methods/upi/intent.md) or the [UPI Collect Flow document](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/s2s-integration/payment-methods/upi/collect.md#step-4-initiate-a-payment) for more details.
...
When you create a registration link, an [invoice](https://razorpay.com/docs/build/llm-docs/payments/invoices.md) is automatically issued to the customer. The customer can use the invoice to complete the mandate registration.

Know how to [create Registration Links](https://razorpay.com/docs/build/llm-docs/payments/recurring-payments/create.md) using the Dashboard.

**INFO**

**Handy Tips**

You can use webhooks to get notifications about successful payments against a registration link. Know more about [Webhooks](https://razorpay.com/docs/build/llm-docs/api/payments/recurring-payments/webhooks.md#check-authorization-link-status-using-webhooks).
