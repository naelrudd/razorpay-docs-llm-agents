...
You can also retrieve the `token_id` via the [payment.failed webhook](https://razorpay.com/docs/build/llm-docs/api/payments/recurring-payments/webhooks.md#payment-failed).
...
For UPI Reserve Pay (SBMD), do not pass the `notification` object when creating an order. The notification object is not supported for Reserve Pay flows. Skip this parameter entirely.
...
Before initiating a debit, it is good practice to check the remaining funds available under the mandate to avoid payment failures. See [Track Mandate Funds](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/s2s-integration/recurring-payments/upi-reserve-pay/manage.md#track-mandate-funds).
