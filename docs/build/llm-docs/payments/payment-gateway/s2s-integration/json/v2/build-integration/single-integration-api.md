...
### Netbanking
    ...
    : `array` Payment capture settings for the payment. The options sent here override the [account level auto-capture settings](https://razorpay.com/docs/build/llm-docs/payments/payments/capture-settings.md) configured using the Dashboard.
      ...
      : `string` Option to automatically capture payment. Possible values:
        ...
        - `manual`: You have to manually capture payments using our [Capture API](https://razorpay.com/docs/build/llm-docs/api/payments.md#capture-a-payment) or from the [Dashboard](https://razorpay.com/docs/build/llm-docs/payments/payments/dashboard.md#manually-capture-payments).
...
If you are a TPV business accepting UPI payments, you must pass `method=upi` in **two places** in this request:
1. At the **order level** (top-level `method` field, alongside `amount` and `currency`).
2. Inside the **`payment` object** (`payment.method`).

Passing `method=upi` only inside the `payment` object is not sufficient. Omitting it from the order level causes an `invalid bank code` error for long-tail banks. Refer to the [TPV S2S UPI Intent Integration](https://razorpay.com/docs/build/llm-docs/payments/third-party-validation/s2s-integration/upi/intent.md) guide for details.
    ...
    -d '{
     ...
     ```php: PHP
     $api = new Api($key_id, $secret);
     $api->order->create(array('amount'=>50000,'currency'=>'INR','method'=>'upi','receipt'=>'receipt#1','notes'=>array('key1'=>'value3','key2'=>'value2',),'partial_payment'=>true,'customer_id'=>'cust_E9penp7VGhT5yt','transfers'=>array(array('account'=>'acc_IRQWUleX4BqvYn','amount'=>1000,'currency'=>'INR','notes'=>array('branch'=>'Acme Corp Bangalore North','name'=>'Gaurav Kumar',),'linked_account_notes'=>array('branch',),'on_hold'=>true,'on_hold_until'=>1671222870,),1=>array('account'=>'acc_IROu8Nod6PXPtZ','amount'=>1000,'currency'=>'INR','notes'=>array('branch'=>'Acme Corp Bangalore South','name'=>'Gaurav Kumar',),'linked_account_notes'=>array('branch',),'on_hold'=>false,),),'products'=>array(),'bank_account'=>array('account_number'=>'765432123456789','name'=>'Gaurav Kumar','ifsc'=>'HDFC0000053',),'payment_config'=>array('capture'=>'automatic','capture_options'=>array('automatic_expiry_period'=>12,'manual_expiry_period'=>7200,'refund_speed'=>'optimum',),),'payment'=>array('amount'=>100,'email'=>'gaurav.kumar@example.com','contact'=>'9090909090','method'=>'upi','upi'=>array('flow'=>'intent',),'ip'=>'192.168.0.103','referer'=>'http','user_agent'=>'Mozilla/5.0','description'=>'Test payment',),'user_agent'=>'Mozilla/5.0',));
    ```
    ...
    ```json: Success
     {
       ...
       "amount_paid": 0,
       "attempts": 10,
       "created_at": 1776678201,
       "currency": "INR",
       "entity": "order",
       "id": "order_SfhZHx8tDcfiv4",
       "notes": {
         "key1": "value3",
         "key2": "value2"
       },
       ...
       "payment_workflow": {
         "next": [
           {
             "action": "intent",
             "url": "upi://pay?am=100.00&cu=INR&mc=8241&mode=04&pa=mehulkhandelwal138051.rzp@rxairtel&pn=MEHULKHANDELWAL&tn=UPIrecurringflowTestingforrearch&tr=ShEWfGVXDGRjei"
           },
           {
             "action": "poll",
             "url": "https://api.razorpay.com/v1/payments/pay_ShEWfGVXDGRjei"
           }
         ],
         "razorpay_payment_id": "pay_ShEWfGVXDGRjei"
       },
       "receipt": "receipt#1",
       "status": "attempted"
     }
    ...
    `currency` _mandatory_
     ...
     `method` _optional_
     : `string` The payment method for the order. Possible values:
        - `netbanking`
        - `upi`
        - `card`
...
If you are a TPV business accepting UPI payments, this field is **mandatory**. Pass `method=upi` here in addition to `payment.method`. Omitting `method` from the order level causes an `invalid bank code` error for long-tail banks, even when `payment.method` is correctly set.
...
If no value is passed, the refund is processed using the [default speed set on the Dashboard](https://razorpay.com/docs/build/llm-docs/payments/refunds.md#setting-the-default-speed-of-refunds).
    ...
    `payment` _mandatory_
        ...
        : `json object` Details of the UPI payment received. Only applicable if method is upi.
          ...
          `flow`
          : `string` The type of UPI flow. Possible value `intent`.

          `vpa`
          : `string` The customer's VPA (Virtual Payment Address) or UPI id used to make the payment. For example, gauravkumar@exampleupi. Required when `flow` is `collect`.
...
### Response Parameters
     ...
     `currency`
     : `string` The currency in which the transaction was made.  See the [list of supported currencies](https://razorpay.com/docs/build/llm-docs/payments/international-payments.md#supported-currencies). Length must be of 3 characters.
...
### 1.1 Handle Payment Success and Error Events
     ...
     #### Failure Callback
     If the payment has failed, the callback will contain details of the error. Refer to [Errors](https://razorpay.com/docs/build/llm-docs/errors.md) for details.
...
- [Late Authorisation](https://razorpay.com/docs/build/llm-docs/payments/payments/late-authorisation.md)
- [Payment Downtime](https://razorpay.com/docs/build/llm-docs/api/payments/downtime.md)
- [Payment Errors](https://razorpay.com/docs/build/llm-docs/errors.md)
...
[Step 2: Test Integration](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/s2s-integration/json/v2/test-integration.md)
