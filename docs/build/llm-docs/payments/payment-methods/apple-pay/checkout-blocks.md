...
- **Order Creation via Backend**: Your server must be able to create orders using the [Orders API](https://razorpay.com/docs/build/llm-docs/api/orders/create.md) and pass the `order_id` to the frontend.
...
Before initiating a payment, create an order on your backend using the [Razorpay Orders API](https://razorpay.com/docs/build/llm-docs/api/orders/create.md). Pass the resulting `order_id` to your frontend to initialise the SDK.
...
Refer to the [Orders API documentation](https://razorpay.com/docs/build/llm-docs/api/orders/create.md) for detailed request and response parameters.
