...
All bank transfer payments are auto-captured. [Manual capture](https://razorpay.com/docs/build/llm-docs/payments/payments/capture-settings.md#manually-capture-payments) of payments is not supported.
...
The bank transfer payment method will appear for the [Payment Gateway](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/web-integration/standard.md) and products such as Payment Links and Payment Pages.
...
### Step 1: Track Checkout Modal Using `ondismiss` Function
     ...
     If you have integrated with [Razorpay Standard Checkout](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/web-integration/standard.md), you must implement the `ondismiss` function to track the lifecycle of the Checkout modal. This displays the `close` icon, which the customer can use to exit the Checkout.
...
If you are using [Android SDK](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/android-integration/standard.md), you can rely on the "payment cancelled by user" error response to track the lifecycle of the Checkout modal.
...
### Step 3: Subscribe to Webhook Event
   ...
   You must subscribe to the `virtual_account.credited` webhook event on the Dashboard to receive notifications whenever customers make payments using bank transfers. Know how to [set up webhooks](https://razorpay.com/docs/build/llm-docs/webhooks/setup-edit-payments.md).
...
### Step 4: Track Checkout Modal Using `ondismiss` Function
   ...
   (Only if you are using [Standard Checkout](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/web-integration/standard.md#manual))
...
### Step 6: Subscribe to Webhook Event
    ...
    You must subscribe to the `virtual_account.credited` webhook event on the Dashboard to receive notifications whenever customers make payments using bank transfers. Know how to [setup webhooks](https://razorpay.com/docs/build/llm-docs/webhooks.md#set-up-webhooks).
...
- [Hosted Integration for Bank Transfer](https://razorpay.com/docs/build/llm-docs/payments/payment-methods/bank-transfer/hosted-integration.md)
- [Custom Integration for Bank Transfer](https://razorpay.com/docs/build/llm-docs/payments/payment-methods/bank-transfer/custom-integration.md)
