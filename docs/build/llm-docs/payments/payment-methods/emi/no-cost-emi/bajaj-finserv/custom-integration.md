...
- Refer to our [web custom integration document](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/web-integration/custom.md).
...
The list of checkout parameters is available [here](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/web-integration/custom/build-integration.md).
...
1. Check if a `payment_ID` has been generated.  If no `payment_ID` has been generated, it means that the transaction has failed.
  

#### Subscribe to Webhook events

You can subscribe to a Webhook event that is generated when a certain event happens in our server. When one of those events is triggered, Razorpay sends the Webhook payload to the configured URL. [Know how to set up Webhooks.](https://razorpay.com/docs/build/llm-docs/webhooks.md)

#### Poll APIs

You can retrieve the status of the payments by polling our [Payment APIs](https://razorpay.com/docs/build/llm-docs/api/payments.md#fetch-multiple-payments).
...
1. Navigate to **Account & Settings** → **API Keys** → **Generate Key** to generate API key for live mode.
