...
- [Generate the API keys](https://razorpay.com/docs/build/llm-docs/api/authentication.md#generate-api-keys).
- Integrate with [Razorpay Custom Checkout](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/web-integration/custom.md).
...
: `string` The unique identifier of the customer. Learn how to create a customer using the [Customers API](https://razorpay.com/docs/build/llm-docs/api/customers.md). This parameter is mandatory if you want to associate the virtual account with a specific customer.
...
`customer_id`
: `string` Unique identifier of the customer to whom the virtual account is linked. Refer the [Customer API](https://razorpay.com/docs/build/llm-docs/api/customers.md) section to learn more.
...
You must subscribe to the `virtual_account.credited` webhook event on the Dashboard to receive notifications whenever customers make payments using bank transfers. Learn how to [setup webhooks](https://razorpay.com/docs/build/llm-docs/webhooks.md#set-up-webhooks).
