...
You can subscribe to the `order.paid`, `payment.authorized` and `payment.captured` Webhook events to get notified once the customer completes the UPI payment. Know more about[Webhooks](https://razorpay.com/docs/build/llm-docs/webhooks.md).

You can also poll our APIs at regular intervals to track the status of the [payments](https://razorpay.com/docs/build/llm-docs/api/payments.md#fetch-payments-based-on-orders) made for an order.
