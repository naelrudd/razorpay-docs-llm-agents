...
The payment processing flow is the same as mentioned [here](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/s2s-integration/json/v2.md). However, you need to make the following changes:
...
The payment processing flow is the same as mentioned [here](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway/s2s-integration/payment-methods.md#emi). However, you need to make the following changes:
- Pass the token number and token expiry values instead of the card number and card expiry values.
- Pass the cryptogram (TAVV) in the `cryptogram_value` field.
