...
`customer_id`
: `string` This is the Razorpay customer id. You can create a token for a specific customer using their customer id. Use the [Customers API](https://razorpay.com/docs/build/llm-docs/payments/customers.md) to create a customer id. This is an optional parameter.
...
: `string` The overall status for the token. Possible values:
  ...
  - `deactivated` : The token attains this state if the token is not active/suspended for any one of the token service providers and is deactivated for at least one token service provider. Know about the complete list of [token states](https://razorpay.com/docs/build/llm-docs/payments/payment-methods/cards/token-hq/merchant-requestor/token-lifecycle.md).
...
Descriptions for the response parameters are present in the [Token Entity parameters table](https://razorpay.com/docs/build/llm-docs/payments/payment-methods/cards/token-hq/dual-token.md#token-entity).
