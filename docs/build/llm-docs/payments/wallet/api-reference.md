...
Refer to the [API Reference](https://razorpay.com/docs/build/llm-docs/api.md) guide to understand the basic concepts of API.

**INFO**

**Handy Tips**

Before creating a wallet, you must create a [customer](https://razorpay.com/docs/build/llm-docs/api/customers.md#create-a-customer).
...
If the customer's wallet has an insufficient balance for the requested payment, the API returns an error. The customer must [load sufficient amount in the wallet](https://razorpay.com/docs/build/llm-docs/payments/wallet/wallet-operations.md#load-a-wallet) to complete the transaction.
...
`utr`
: `string` A unique transaction reference (UTR) number generated for all transactions. You can obtain UTR from the [`payout.updated` webhook payload](https://razorpay.com/docs/build/llm-docs/webhooks.md).
