...
You can start accepting payments by integrating your website, app, or ecommerce store with the Razorpay Payment Gateway through any of the [integration methods available](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway.md).
...
### Payments API
     ...
     Payments APIs are used to capture and fetch payments. The payment entity has one extra field with Optimizer to identify the payment provider through which the payment is processed.
      ...
      Refer to [Payments Entity](https://razorpay.com/docs/build/llm-docs/api/payments.md#payments-entity) for the rest of the parameters.
...
### Webhooks
     ...
     You can use Webhooks to configure and receive notifications when a specific event occurs. When an event is triggered, we send an HTTP POST payload in JSON to the webhook's configured URL. You can [set up Webhooks](https://razorpay.com/docs/build/llm-docs/webhooks/setup-edit-payments.md) from the Dashboard.

      Given below is the sample payload for webhook events for Optimizer. All the parameters and events will remain the same as shown in the [sample payloads for payment webhooks](https://razorpay.com/docs/build/llm-docs/webhooks/payments.md) except for one additional parameter, `gateway_provider`.
...
The Razorpay Dashboard offers a self-serve flow to [add Payment Providers](https://razorpay.com/docs/build/llm-docs/payments/optimizer/add-payment-providers.md) by submitting your details. This process is secure, and the details you add are fully encrypted which is only visible on your Dashboard. Know more about [Razorpay Security](https://razorpay.com/docs/build/llm-docs/security.md).

## Step 3: Configure Routing Rules
Create your rules on the Razorpay Dashboard to [dynamically route transactions](https://razorpay.com/docs/build/llm-docs/payments/optimizer/dynamic-routing.md) using different parameters like payment method, amount, issuer, and more. You can also add priorities in every rule to ensure transactions are routed to the best-performing gateway.

### Related Information

- [Add a Payment Provider](https://razorpay.com/docs/build/llm-docs/payments/optimizer/add-payment-providers.md)
- [Dynamic Routing](https://razorpay.com/docs/build/llm-docs/payments/optimizer/dynamic-routing.md)
- [Capture and Refund Settings](https://razorpay.com/docs/build/llm-docs/payments/optimizer/capture-refund-settings.md)
