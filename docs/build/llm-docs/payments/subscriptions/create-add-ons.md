...
### Example
Consider, Acme Corp. is providing DTH services. A customer wants to add the sports channel pack, which costs an additional 300, to his Subscription only for the next month.

Before the next billing cycle, Acme Corp. can add 300 to the customer's current bill amount as an add-on.

Add-ons can be created:
- [Using APIs](https://razorpay.com/docs/build/llm-docs/api/payments/subscriptions.md#create-an-add-on).
...
You can create add-ons for the Subscriptions using [Create an Add-on](https://razorpay.com/docs/build/llm-docs/api/payments/subscriptions.md#create-an-add-on) API.
