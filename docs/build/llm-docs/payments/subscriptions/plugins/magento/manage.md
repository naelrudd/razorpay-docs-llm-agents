...
When a subscription has been put on hold, you can use the **reactivate** option to restart the subscription once you successfully charge the customer's card. This changes the subscription status to **active** on the Razorpay Dashboard. Note that the **reactivate** button will only be displayed where the payment is not due.
...
If you need to refund money to a customer for any reason, you need to do this manually from the [Razorpay Dashboard](https://razorpay.com/docs/build/llm-docs/payments/payments/dashboard.md#issue-refunds).
