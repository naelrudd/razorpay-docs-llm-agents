...
: `string` The currency in which the customer makes the transaction. See the [list of supported currencies](https://razorpay.com/docs/build/llm-docs/payments/international-payments.md#supported-currencies). The default is `INR`, and the length must be 3 characters.
...
`currency` 
: `string` A 3 letter ISO code for the currency you want to accept the payment. For example, INR. Refer to the list of [supported currencies](https://razorpay.com/docs/build/llm-docs/payments/international-payments.md#supported-currencies).
...
`rto_reasons` 
: `array` Top 5 reasons for risky orders in descending order of importance. [Refer to the list of possible reasons](https://razorpay.com/docs/build/llm-docs/payments/magic-checkout/rto-intelligence/appendix.md) for risky orders.
...
You must update the payment method details if you are not a [Razorpay Payment Gateway](https://razorpay.com/docs/build/llm-docs/payments/payment-gateway.md) user.
