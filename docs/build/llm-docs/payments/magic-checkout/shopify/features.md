...
- [Orders Not Placed](#orders-not-placed)
...
### COD Intelligence
         ...
         You can disable **COD Intelligence** if required. This will enable the COD option for all the customers, including those considered risky by Magic Checkout. Once you enable COD intelligence, the **RTO Analytics** tab will appear. Know more about [RTO Analytics](https://razorpay.com/docs/build/llm-docs/payments/magic-checkout/analytics/rto.md).
...
### Manually Review COD Orders
         ...
         Once you enable manual review, you can review the COD orders and take necessary actions. Refer to the [COD Orders](https://razorpay.com/docs/build/llm-docs/payments/magic-checkout/order-settings/review-cod-orders.md#cod-orders) section for the next steps.
...
Converting orders to prepaid can help you increase order commitment, reduce RTO, streamline operations and enhance customer trust. Know how to [convert COD orders to prepaid](https://razorpay.com/docs/build/llm-docs/payments/magic-checkout/order-settings/prepay-cod.md).
...
If you configure the shipping setting on the Razorpay Dashboard, any shipping setting configured on any plugins or your ecommerce platform will not apply.
         ...
         6. You can add products to the category in two ways. 
             
                
                    1. Enter the **Category name**.
                    2. Select the products of your choice manually.
                    3. Click **Confirm**.
                    
                
                
                    1. Enter the **Category name**.
                    2. Upload a CSV file containing the product IDs you want to assign to this category. Drag the file or click **Upload**.
                         
**INFO**

**Handy Tips**
- Download the sample CSV file to view the accepted format.
- Upload the file in .CSV format.
- If you do not have your product IDs handy, you can export them from Shopify using the [Matrixify app](https://apps.shopify.com/excel-export-import).

                         

                    3. If the uploaded file contains duplicate product IDs or products already assigned to another category, the system displays the following warnings:
                         - **Duplicates found in your file**: Lists the number of duplicate product IDs detected in the file. To remove duplicates, click **Auto remove duplicates and continue**.
                             
                         - **Product conflicts detected**: Lists the products already assigned to other shipping categories. Click **Download conflict report** to review the conflicting products and their current category assignments. To proceed, click **Skip conflicting products and continue**, the conflicting products will be excluded from the new profile and their existing category assignment will be retained.
...
Each product can only belong to one category at a time. If your file contains products already assigned to another category, they will be flagged as conflicts during upload.
...
Razorpay Magic SSO (Single Sign-On) offers a seamless login experience, helping you enhance conversions and gain valuable insights into user behaviour. Know more about how to setup [login with Razorpayt](https://razorpay.com/docs/build/llm-docs/payments/magic-checkout/login-with-razorpay.md) on your shopify store.

## Orders Not Placed

When a customer completes payment on Magic Checkout but the corresponding order fails to be created on Shopify, the order is marked as **not placed**. Common reasons include out-of-stock items, validation errors (such as invalid email or phone number), or an amount mismatch between Magic Checkout and Shopify.

    
### To view and manage not placed orders:

         1. Log in to the Dashboard and navigate to **Magic Checkout** → **Orders**.
         2. Click **All Orders** → **Not Placed** tab.
         3. Search for a specific order using the customer's **Phone Number** or **Email**.
         4. Select the order you want to refund in the **Action** column and click **Refund**.
...
Once you initiate a refund, the amount is returned to the customer's original payment method. Ensure you verify the order details before proceeding.

        

## Inventory and Stock

When a customer completes payment on Magic Checkout, there is a small window between payment confirmation and final order placement on Shopify during which an item in the cart may go out of stock. You can configure whether Magic Checkout should place the order regardless of stock availability or automatically refund the customer if an item goes out of stock. 

Know more about [Inventory and Stock Settings](https://razorpay.com/docs/build/llm-docs/payments/magic-checkout/order-settings/inventory-stock.md).
...
Know how to [create offers](https://razorpay.com/docs/build/llm-docs/payments/offers/create.md).

## Shopify Bundles 

Shopify Bundles is a Shopify app that lets businesses create and sell combinations of products together, often at a discounted price, helping increase average order value (AOV). Magic Checkout now supports Shopify Bundles. Customers with bundle products in their cart can complete their purchase entirely within Magic Checkout, without being redirected to the native Shopify checkout.

## Zero Rupee Orders

A zero rupee order occurs when the final payable amount on a customer's cart is reduced to ₹0 after applying a 100% discount. This can happen when a customer uses a combination of coupons, gift cards, or loyalty coins that covers the entire order value.

For example, if a customer's cart value is ₹500 and they apply a gift card worth ₹500, the final amount payable becomes ₹0.

Previously, Magic Checkout displayed ₹1 and opened the payment gateway for such orders. Now, when the order amount is ₹0, the payment gateway is skipped entirely and the customer sees a **Place Order** button directly on the checkout screen to complete their purchase.
...
[Troubleshooting and FAQs](https://razorpay.com/docs/build/llm-docs/payments/magic-checkout/troubleshooting-faqs.md#woocommerce)
