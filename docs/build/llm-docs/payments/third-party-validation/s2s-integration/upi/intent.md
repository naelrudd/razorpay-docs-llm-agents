...
4. **Mobile site**  
  ...
  If you have higher traffic via mobile site, make sure you provide the option of UPI intent payment to your users using [our m-site integration](https://razorpay.com/docs/build/llm-docs/payments/payment-methods/upi/google-pay/custom-integration.md). This will help you achieve better success rates.
5. **Gpay SDK** 

  If your UPI transaction volumes are high, you can also explore integrating [Gpay SDK](https://razorpay.com/docs/build/llm-docs/payments/payment-methods/upi/google-pay.md#android-integration-using-google-pay-sdk). This provides a better user experience and about a 3-5% improvement in success rate.
...
- [Generate API Keys from the Dashboard](https://razorpay.com/docs/build/llm-docs/api/authentication.md#generate-api-keys) if you have not done already.
- Configure the [payment capture settings](https://razorpay.com/docs/build/llm-docs/payments/payments/capture-settings.md) on the Dashboard.
...
If you are a TPV business using the Consolidated Payment API, you must pass `method=upi` in **both** of the following:
1. The **Create Order** request (`/v1/orders`) shown in the [sample above](#step-1-create-an-order).
2. The **Initiate Payment** request (`/v1/payments/create/upi`) covered in [Step 2](#step-2-initiate-a-payment).

Omitting `method=upi` from the payment initiation request causes an `invalid bank code` error, particularly for long-tail banks. This applies even if `method` is correctly set at the order level.
...
**`method` is Required at Both Order and Payment Level for TPV**

For TPV businesses, `method=upi` must be explicitly passed here in the payment initiation request, in addition to the Create Order request in Step 1. This is mandatory for correct bank code resolution across all banks, including long-tail banks.
