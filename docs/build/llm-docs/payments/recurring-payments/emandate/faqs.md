...
### 5. What is the timeline for Emandate token registration confirmation?
         ...
         **Confirmation Timelines by Authentication Type and Bank**
          ...
          ---
          Debit Card, Netbanking & Aadhaar | All Banks (NPCI eNACH) | Real-time | Once an Emandate registration request is created, the token status confirmation happens immediately in real-time.
          ---
...
### 6. How can I cancel my customer's registered mandates?
         ...
         You can initiate cancellation and track the status by following these steps:
          - Delete the mandate token using [Dashboard](https://razorpay.com/docs/build/llm-docs/payments/recurring-payments/create.md#delete-the-token) or [APIs](https://razorpay.com/docs/build/llm-docs/api/payments/recurring-payments/emandate/tokens.md#23-delete-tokens). 
          - Razorpay will initiate cancellation for the deleted tokens on the next working day.
          - On acknowledgement from NPCI for cancellation, you will receive a 'Token Cancelled' [webhook](https://razorpay.com/docs/build/llm-docs/api/payments/recurring-payments/webhooks.md#token-cancelled), which can be taken as a confirmation of mandate cancellation.
...
### 7. Can Razorpay help schedule debits for Emandates?
         ...
         Know more about [Subscriptions](https://razorpay.com/docs/build/llm-docs/payments/subscriptions.md).
...
### 12. Which banks are supported for Emandates?
         ...
         You can refer to the [Supported Banks](https://razorpay.com/docs/build/llm-docs/payments/recurring-payments/emandate/supported-banks.md) page for the list of supported banks for Emandates.
...
### 15. How do I enable Aadhaar as an auth type for emandates? Do I need to make changes in my Integration to use the aadhaar emandate?
         ...
         Aadhaar emandate is available by default for all Razorpay accounts. Pass `auth_type=aadhaar` in the existing emandate APIs to start using Aadhaar-based mandate registration.
        

    
### 16. How can I modify or edit a mandate that has already been created?
...
### 2. What is the maximum amount which can be debited from the customers?
         ...
         If you do not set a limit for the mandate, the maximum limit defaults to ₹99,999 for Emandates.
         ...
         **Auth Type** | **Maximum Allowed Limit** | Default Allowed limit(if no max amount mentioned)
         ---
         Netbanking | ₹1,00,00,000 | ₹99,999
         ---
         Debit Card | ₹1,00,00,000 | ₹99,999
         ---
         Aadhaar | ₹1,00,00,000 | ₹99,999
         ...
         **Auth Type** | **Maximum Allowed Limit** | Default Allowed limit(if no max amount mentioned)
         ---
         Netbanking | ₹1,00,000 | ₹99,999
         
        

    
### 3. Is there a cooling period for Emandates registered via NPCI ENACH (debit card, netbanking and Aadhaar) for initiating the first debit after completing the registration process?
...
### 4. Is there a cooling period for Emandates created via Direct Bank Integration (ICICI, Axis, HDFC, SBI) for initiating the first debit after completing the registration process?
...
### 5. For Emandates, how long does it take a subsequent charge to move from the `created` state to the `authorized` state?
         ...
         ---
         All Banks under NPCI ENACH (debit card, netbanking, Aadhaar) | T+2 working days | Debit requests created between **T-1 day 9 AM** and **T day 9 AM** are processed on **T by end of the day**. Requests after **T day 9 AM** are processed in **T+1 cycle**.
         ---
...
### 6. Will my debits get processed on holidays as well?
...
### 7. I am getting an error saying `Customer has to refer to branch to complete registration` while initiating debits for my customers?
...
### 8. I am getting an error saying  `Payment failed because emandate is cancelled or inactive` while presenting debits for my customers?
...
### 9. The payment status for debit presentment says `authorized` and I have not received a settlement for this payment yet. Why?

         Payments have to be `captured` for the corresponding settlement to take place. You can manually capture the payment from the [Dashboard](https://razorpay.com/docs/build/llm-docs/payments/payments/capture-settings.md#manually-capture-payments).

         Alternatively, to avoid manual dependency, you can enable Auto Capture for all your payments from the Dashboard.

         Know more about [Capture Settings](https://razorpay.com/docs/build/llm-docs/payments/payments/capture-settings.md#auto-capture-all-payments).
        

    
### 10. When can I retry subsequent debit if the first attempt fails for NPCI ENACH?
...
### 11. Will there be any charges applied for subsequent debit failures?
